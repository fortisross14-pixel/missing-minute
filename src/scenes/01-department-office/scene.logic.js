export function interactOffice({ hotspotId, verb, selectedItem, state, api }) {
  ...
  const invalidPickup = (text='I don’t think I can pick that up.') => api.say(text);

  if (hotspotId === 'pindle') {
    if (verb === 'talk' || verb === 'use') return api.dialogue('office.pindle');
    if (verb === 'pickup') return invalidPickup('I don’t think I can pick up Mr. Pindle. He has seniority and surprisingly good desk grip.');
    if (verb === 'give') return api.say(selectedItem ? `Mr. Pindle looks at ${selectedItem.replaceAll('-',' ')} and begins searching for the correct refusal form.` : 'I should select something before attempting a formal transfer of property.');
    return look('Mr. Pindle is pretending to read a blank page with impressive concentration.');
  }

  if (hotspotId === 'terminal') {
    if (f.drillTriggered && !f.packageOpened && ['use','pickup'].includes(verb)) return api.dialogue('office.package');
    ...
    if (verb === 'pickup') return invalidPickup('The terminal is bolted to the counter. Bureaucracy has finally secured something.');
  ...
  if (hotspotId === 'poster') {
    if (verb === 'pickup') return invalidPickup('It is glued to the wall with the kind of commitment the Department usually discourages.');
  ...
  if (hotspotId === 'clock') {
    if (verb === 'pickup') return invalidPickup('The official clock is too large, too high, and legally considered part of the building’s personality.');
  ...
  if (hotspotId === 'alarm') {
    ...
    if (verb === 'pickup') return invalidPickup('The alarm is attached to the wall. The interesting detachable part is missing.');
  ...
  if (hotspotId === 'handle') {
    ...
    if (verb === 'pickup') {
      api.setFlags({ alarmDiscovered:true });
      api.thought('I need something long enough to reach it without putting my hand in classified fish water.');
      return api.say('I don’t want to get my hands wet. Especially not in evidence water.');
    }
    return look('The missing fire-alarm handle is sitting at the bottom of the bowl, looking annoyingly reachable.');
  }
  ...
  if (hotspotId === 'fishbowl') {
    ...
    if (verb === 'pickup') return invalidPickup('I am not carrying an entire fishbowl through a government office. Mr. Ledger has suffered enough.');
  ...
  if (hotspotId === 'map-exit') {
    ...
    if (verb === 'pickup') return invalidPickup('It is a wall map. I already have a smaller, more portable crisis.');
    api.changeScene('02-gannets-end-harbor');
    return;
  }

  if (hotspotId === 'forms') {
    if (verb === 'pickup') return invalidPickup('I already have one complaint form. Collecting more would suggest enthusiasm.');
    return look('Rejected forms, obsolete forms, and one form requesting permission to reject obsolete forms.');
  }
  if (hotspotId === 'desk') {
    if (verb === 'pickup') return invalidPickup('My desk is not technically mine, and it is heavier than my career prospects.');
    return look('Mara’s desk: one lamp, several stamps, and a career developing at administrative speed.');
  }
  if (hotspotId === 'shelf') {
    if (verb === 'pickup') return invalidPickup('I can pick up something from the shelf. Picking up the shelf itself feels ambitious.');
    return look('Lost umbrellas, unmatched gloves and a bowling pin whose colleagues left to pursue individual careers.');
  }
